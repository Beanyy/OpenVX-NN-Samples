...
def detect(output_filename, img_filename, mode):
    """ given a YOLO caffe model and an image, detect the objects in the image
    """
    out = np.fromfile(output_filename, dtype=np.float32)
    out = np.reshape(out, (1, 425, 12, 12))

    img_cv = cv2.imread(img_filename)
    results = get_candidate_objects(out[0], img_cv.shape, mode)
...
def main():
    ...
    parser.add_argument('raw_output', type=str, help='model output')
    ...
    detect(args.raw_output, args.image, args.mode)
